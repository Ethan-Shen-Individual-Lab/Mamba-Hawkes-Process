...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        # Loss function with configurable weights (same as Main.py)
        beta = opt.beta
        gamma = opt.gamma
        ...
        # Post-update parameter check for Retweet dataset (exactly same as Main.py)
...
def eval_epoch(model, validation_data, pred_loss_func, opt):
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            # Basic numerical stability for time intervals (exactly same as Main.py)
            event_time = torch.clamp(event_time, min=1e-6, max=1e6)
...
def train(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt, log_file):
    ...
    # Create train log file for ablation
...
def get_experiment_configs():
    """ Get all experiment configurations - same as Main.py """
...
def run_ablation_experiment(dataset_name, config, ablation_mode, epochs=40, beta=1.0, gamma=1e-4):
    """ Run ablation experiment with specified ablation mode """
    
    print(f"\n{'='*70}")
    print(f"Running ABLATION: {dataset_name} with {ablation_mode}")
    print(f"Dataset: {config['file']}")
    if ablation_mode == 'time_scaling_only':
        print("Variant: Time-Scaling Only (GRU scaling, no dual-channel)")
        print("Time prediction: adjusted by scaling factor")
    elif ablation_mode == 'dual_channel_only':
        print("Variant: Dual-Channel Only (event 2/3 + time 1/3, no GRU scaling)")
        print("Time prediction: no scaling factor adjustment")
    print(f"{'='*70}")
    ...
    class Opt:
        def __repr__(self):
            return f"Ablation Config(dataset={self.data.split('/')[-2] if '/' in self.data else 'unknown'}, " \
                   f"ablation={ablation_mode}, d_model={self.d_model}, batch_size={self.batch_size}, lr={self.lr}, " \
    ...
    try:
        ...
        # Prepare ablation model
        dataset_type = 'so' if 'so' in dataset_name.lower() else 'retweet' if 'retweet' in dataset_name.lower() else 'default'
        config_mamba = MambaConfig(d_model=opt.d_model, n_layers=opt.n_layers, dataset_type=dataset_type, ablation_mode=ablation_mode)
        ...
        # Enhanced weight initialization for all datasets (exactly same as Main.py)
        with torch.no_grad():
            for name, param in model.named_parameters():
        ...
        # Optimizer with Retweet-specific learning rate (exactly same as Main.py)
        if 'retweet' in dataset_name.lower():
        ...
        # Prediction loss function with label smoothing
        pred_loss_func = Utils.LabelSmoothingLoss(0.1, num_types, ignore_index=-1)
        ...
        print(f"Ablation experiment {dataset_name} ({ablation_mode}) completed successfully!")
        
    except Exception as e:
        print(f"Error in ablation experiment {dataset_name} ({ablation_mode}): {str(e)}")
        import traceback
        traceback.print_exc()

def main():
    """ Main function - Run ablation experiments """
    
    # Add command line argument parsing
    parser = argparse.ArgumentParser(description='A-MHP Ablation Study')
    ...
    print("Starting A-MHP Ablation Study")
    print("Testing: Time-Scaling Only vs Dual-Channel Only")
    print("=" * 70)
    ...
    if args.dataset != 'all':
        ...
        print(f"Running ablation on single dataset: {args.dataset}")
    else:
        print(f"Running ablation on all datasets")
    
    # Ablation variants to test
    ablation_modes = ['time_scaling_only', 'dual_channel_only']
    
    total_experiments = len(experiment_configs) * len(ablation_modes)
    print(f"Total ablation experiments: {total_experiments}")
    print(f"Datasets: {', '.join(experiment_configs.keys())}")
    print(f"Variants: {', '.join(ablation_modes)}")
    print("=" * 70)
    ...
    for ablation_mode in ablation_modes:
        for dataset_name, config in experiment_configs.items():
            ...
            try:
                run_ablation_experiment(dataset_name, config, ablation_mode, epochs=40,
                                      beta=args.beta, gamma=args.gamma)
            except KeyboardInterrupt:
                print("\nAblation experiments interrupted by user")
                return
            except Exception as e:
                print(f"Failed ablation experiment {dataset_name} ({ablation_mode}): {str(e)}")
                continue
