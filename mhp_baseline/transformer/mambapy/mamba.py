...
class MambaBlockdelta(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        # projects x to input-dependent delta, B, C
        self.x_proj = nn.Linear(config.d_inner, 2 * config.d_state, bias=False)
    ...
    def ssm(self, x, z, delta):
        ...
        deltaBC = self.x_proj(x) # (B, L, dt_rank+2*N)
        B, C = torch.split(deltaBC, [self.config.d_state, self.config.d_state], dim=-1) # (B, L, dt_rank), (B, L, N), (B, L, N)
        _, B, C = self._apply_layernorms(None, B, C)
        diff = torch.diff(delta, dim=1)  # shape: B*(L-1)

        # 将原来event_time的第一项添加到最前面
        first_col = delta[:, :1]  # shape: B*1
        new_tensor = torch.cat((first_col, diff), dim=1)  # shape: B*L

        # 复制D次
        new_tensor = new_tensor.unsqueeze(2).repeat(1, 1, ED)  # shape: B*L*D

        # 交换L和D的位置
        deltat = new_tensor.transpose(1, 2)
        #delta = self.dt_proj.weight @ delta.transpose(1, 2) # (ED, dt_rank) @ (B, L, dt_rank) -> (B, ED, L)
        # here we just apply the matrix mul operation of delta = softplus(dt_proj(delta))
        # the rest will be applied later (fused if using cuda)
        ...
        if self.config.use_cuda:
            ...
            # "softplus" + "bias" + "y * silu(z)" operations are fused
            y = self.selective_scan_cuda(x, deltat, A, B, C, D, z=z, delta_softplus=True, delta_bias=self.dt_proj.bias.float())
            y = y.transpose(1, 2) # (B, L, ED)
        
        else:
            deltat = deltat.transpose(1, 2)
            deltat = torch.clamp(F.softplus(deltat), min=1e-3, max=1)
            
            if self.config.pscan:
                y = self.selective_scan(x, deltat, A, B, C, D)
            else:
                y = self.selective_scan_seq(x, deltat, A, B, C, D)
